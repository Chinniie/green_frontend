  ...
  return (
    <Layout>
      <div className="max-w-[1600px] mx-auto space-y-8 md:space-y-12 pb-24 px-2 md:px-6 animate-in fade-in duration-1000">
        {/* --- 🔝 Header: ปรับพื้นหลังให้สลับ White (Light) และ Slate-950 (Dark) --- */}
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-white dark:bg-slate-950 p-8 md:p-12 rounded-[2.5rem] md:rounded-[4rem] shadow-xl border border-slate-100 dark:border-white/5 relative overflow-hidden group transition-all duration-500">
          <div className="relative z-10">
            {/* แก้ไข H1: ใช้ leading-tight และ py-2 เพื่อไม่ให้สระไทยถูกตัด */}
            <h1 className="text-3xl md:text-5xl font-black text-slate-900 dark:text-white tracking-tighter mb-3 italic uppercase leading-tight py-2 transition-colors duration-300">
              {t("machineIntelCenter")}
            </h1>

            <p className="text-emerald-500 dark:text-emerald-400 font-black uppercase tracking-[0.2em] text-[10px] flex items-center gap-2">
          ...
          {/* Metrics Side: ใช้ Glassmorphism ที่ปรับตามโหมด */}
          <div className="flex flex-wrap gap-4 relative z-10 mt-8 lg:mt-0">
            {/* Card 1 */}
            <div className="flex-1 lg:flex-none px-8 py-5 bg-slate-50 dark:bg-white/5 backdrop-blur-md rounded-[2rem] border border-slate-100 dark:border-white/10 text-center min-w-[160px] transition-colors">
              <p className="text-[9px] text-slate-400 dark:text-white/50 uppercase font-black mb-1 tracking-widest">
                {t("totalActiveAssets")}
              </p>
          ...
          {/* Big Decorative Background Text: Scoped Inside Container */}
          <div className="absolute -right-10 -bottom-10 text-[15rem] md:text-[20rem] text-slate-100 dark:text-white/[0.03] pointer-events-none italic font-black select-none transition-colors duration-500">
            4.0
          </div>
        </div>
      </div>
