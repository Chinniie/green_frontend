...
export default function Simulation() {
  ...
  if (loading)
    return (
      <Layout>
        <div className="p-20 text-center animate-pulse text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest text-xs">
          🧪 {t("preparingLab")}
        </div>
      </Layout>
    );
    ...
    <Layout>
      <div className="max-w-6xl mx-auto space-y-10 pb-20 p-6 md:p-10">
        ...
        <header className="flex flex-col md:flex-row justify-between items-end gap-6">
          <div>
            <nav className="flex items-center gap-2 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-4 italic">
              <span>LAB</span>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium opacity-70">
              {t("simLabSub")}
            </p>
          </div>

          <div className="bg-white dark:bg-slate-800 px-8 py-4 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 hidden md:block">
        ...
        {/* SLIDER */}
        <div className="bg-slate-900 dark:bg-slate-950 p-12 rounded-[4rem] text-white shadow-2xl relative overflow-hidden group">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* CARD 1 */}
          <div className="bg-white dark:bg-slate-800 p-10 rounded-[3.5rem] shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col justify-between group hover:shadow-2xl transition-all">
            <div>
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-10 italic">
                {t("monthlySavings")}
              </p>
            ...
            <div className="mt-10 p-6 bg-emerald-50 dark:bg-emerald-500/10 rounded-[2.5rem] border border-emerald-100 dark:border-emerald-500/20 italic">
          ...
          {/* CARD 2 (gradient ไม่ต้องแก้เยอะ) */}
          ...
          {/* CARD 3 */}
          <div className="bg-slate-50 dark:bg-slate-900 p-10 rounded-[3.5rem] border border-slate-100 dark:border-slate-700 flex flex-col justify-between">
            <div className="space-y-6">
              <div>
                <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1 italic">
                  {t("simulatedUsage")}
                </p>
            ...
            <button className="w-full mt-10 py-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-sm hover:bg-slate-900 hover:text-white dark:hover:bg-emerald-500 transition-all active:scale-95">
              {t("commitGoal")}
            </button>
          </div>
        </div>
        ...
        {/* SUGGESTION */}
        <div className="bg-white dark:bg-slate-800 p-12 rounded-[4rem] shadow-sm border border-slate-100 dark:border-slate-700">
          <h3 className="text-2xl font-black text-slate-800 dark:text-white mb-10 flex items-center gap-4 uppercase tracking-tighter">
            <div className="w-14 h-14 bg-emerald-100 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 rounded-2xl flex items-center justify-center text-3xl shadow-inner">
              💡
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div
                key={i}
                className="p-8 bg-slate-50 dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 hover:scale-105 transition-all group"
              >
                <div className="mb-4">
                  <span className="bg-white dark:bg-slate-800 px-3 py-1 rounded-full text-[9px] font-black text-slate-400 dark:text-slate-500 shadow-sm border border-slate-100 dark:border-slate-700 tracking-widest uppercase">
                    {t("impactLabel")}{" "}
                    <span className={tip.color}>{tip.impact}</span>
                  </span>
                </div>

                <h4 className="font-black text-slate-800 dark:text-white text-lg mb-2 uppercase tracking-tight">
                  {tip.title}
                </h4>

                <p className="text-xs text-slate-400 dark:text-slate-500 leading-relaxed font-medium opacity-80">
                  {tip.desc}
                </p>
              </div>
            ))}
