...
export default function Dashboard() {
  ...
  // --- Data Fetching (Relative to User ID) ---
  ...
  // --- Relative Calculations (Memoized) ---
  const stats = useMemo(() => {
    const defaultStats = {
      totalKwh: 0,
      totalCost: 0,
    ...
    const carbonFactor = config?.carbon_factor || 0.507;

    // หา Peak Hour แบบ Relative ตามข้อมูลจริง
    const peak = [...data].sort((a, b) => Number(b.kwh) - Number(a.kwh))[0];

    // จัดกลุ่มอุปกรณ์
    const deviceMap = data.reduce((acc, curr) => {
      const name = curr.device || "Other Assets";
      acc[name] = (acc[name] || 0) + Number(curr.kwh || 0);
      return acc;
    }, {});

    const topDevice = Object.entries(deviceMap)
      .map(([name, kwh]) => ({ name, kwh }))
      .sort((a, b) => b.kwh - a.kwh)[0];

    return {
      totalKwh,
      totalCost: totalKwh * effectiveRate,
      carbon: totalKwh * carbonFactor,
      peak,
      topDevice,
    };
  ...
  return (
    <Layout>
      {/* 🧠 AI Onboarding - Fixed position relative to viewport */}
      {showTutorial && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-6">
          <div className="bg-white dark:bg-slate-800 rounded-[3rem] p-10 max-w-sm w-full text-center shadow-2xl border border-slate-100 dark:border-slate-700 animate-in zoom-in duration-300">
            ...
            <h3 className="text-2xl font-black mb-3 text-slate-900 dark:text-white uppercase tracking-tighter italic">
              Welcome, {user.username || "Boss"}!
            </h3>
            <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed mb-8 font-medium">
              Dashboard ใหม่พร้อมแล้วครับ
              ระบบวิเคราะห์จุดใช้ไฟที่สูงที่สุดให้คุณแบบ Real-time
            </p>
            <button
              onClick={() => {
              ...
              className="w-full py-4 bg-slate-900 dark:bg-emerald-500 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-[1.02] transition-all"
            >
              รับทราบครับ
            </button>
          </div>
      ...
      <div className="max-w-[1600px] mx-auto pb-24 space-y-12 animate-in fade-in duration-1000">
        {/* --- 🔝 Header Section --- */}
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 px-2 relative">
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className={`p-3 rounded-2xl bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700 transition-all duration-500 ${refreshing ? "animate-spin text-emerald-500" : "text-slate-400 hover:rotate-180"}`}
              >
                <RefreshCw size={18} />
              </button>
            </div>
          ...
          <div className="flex bg-white dark:bg-slate-800 p-2 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-700 w-full md:w-auto">
            {["baht", "kwh"].map((mode) => (
              <button
                key={mode}
                onClick={() => setViewMode(mode)}
                className={`flex-1 md:flex-none px-8 py-3 rounded-2xl text-[10px] font-black transition-all uppercase tracking-widest ${viewMode === mode ? "bg-slate-900 dark:bg-emerald-500 text-white shadow-xl" : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"}`}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="lg:col-span-3 space-y-10">
            ...
            <div className="bg-white dark:bg-slate-800/50 p-6 md:p-12 rounded-[3.5rem] shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden relative group">
          ...
          {/* ⚡ Peak Card - Relative scoped overflow */}
          <div className="bg-slate-900 dark:bg-slate-950 p-10 rounded-[4rem] shadow-2xl text-white relative overflow-hidden flex flex-col justify-between border border-white/5 min-h-[500px] group">
            ...
            <div className="relative z-10 py-10">
              ...
              <h4 className="text-7xl md:text-8xl font-black tracking-tighter italic leading-none group-hover:scale-105 transition-transform duration-700">
            ...
            <button
              ...
              className="relative z-10 w-full py-5 bg-white dark:bg-emerald-500 text-slate-900 dark:text-white rounded-[2rem] text-[11px] font-black uppercase tracking-[0.2em] hover:bg-emerald-400 transition-all flex items-center justify-center gap-2"
            >
              {t("deepAnalytic")} <ChevronRight size={14} />
            </button>

            {/* Decorative - Scoped to this relative parent */}
            <div className="absolute -right-20 -bottom-20 w-80 h-80 bg-emerald-500/10 rounded-full blur-[100px] pointer-events-none group-hover:bg-emerald-500/20 transition-all duration-1000" />
          </div>
        </div>
